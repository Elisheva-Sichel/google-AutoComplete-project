...
def fix_mistake(str_input, fixed_list, k_best):
    ...
    # 1 score
    fixed_list += list(
        sorted(add_and_check_dup(fixed_list, swap_char(str_input, 5), scores - 1), key=lambda x: x.completed_sentence))[
                  :k_best]
    index = 4
    score_to_remove = 1
    for i in list(range(5))[::-1]:
        if len(fixed_list) < k_best:
            fixed_list += list(
                sorted(add_and_check_dup(fixed_list, swap_char(str_input, index, index + 1), scores - score_to_remove ),
                       key=lambda x: x.completed_sentence))[
                          :k_best]
        if len(fixed_list) < k_best:
            # 2 scores
            buf = add_or_delete_char(str_input, index, index+1)
            buf += swap_char(str_input, index - 1, index)
            index -= 1
            fixed_list += list(sorted(add_and_check_dup(fixed_list, buf, scores - score_to_remove - 1)[:k_best - len(fixed_list)],
    ...
    #                 # 5scores
    #                 fixed_list += list(
    #                     sorted(add_and_check_dup(fixed_list, swap_char(str_input, 0, 1), scores - 5),
    #                            key=lambda x: x.completed_sentence))[:k_best - len(fixed_list)]
    #                 if len(fixed_list) < k_best:
    #                     # 6scores
    #                     fixed_list += list(
